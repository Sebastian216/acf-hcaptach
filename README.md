...
2. Rename placeholder strings in file names and content such as `hcaptcha` manually or with the helper script: 
    - `cd` into the directory and run `npm ci` (requires [Node.js](https://nodejs.org/)).
    - Run `npm run rename` and follow the prompts.
3. Copy the renamed `acf-hcaptcha` directory into your theme or plugin.
4. Add this code to your theme or plugin to initialize the new field type, replacing 'acf-hcaptcha' with the new folder name:

    ```php
    include_once __DIR__ . '/acf-hcaptcha/init.php';    
    ```
...
- Change your field type's settings and behavior by editing `class-company-or-project-name-acf-field-hcaptcha.php`.
...
If you remove `field.css` or `field.js`, be sure to also remove the `wp_enqueue` calls in `class-company-or-project-name-acf-field-hcaptcha.php`.

If you plan to use the CSS and JS files, update the '1.0' version number in `class-company-or-project-name-acf-field-hcaptcha.php` to use your theme or plugin version number or constant. This helps to invalidate browser caches for your field type assets when you update your theme or plugin.
